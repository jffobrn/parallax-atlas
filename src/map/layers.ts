 ...
 * MapView owns the map and the store; this owns the drawing. Each image with a
 * vantage casts a ray; where rays cross, the object is resected. Convergence is
 * the image complex.
 */
...
export function buildMapLayers(args: BuildLayersArgs): Layer[] {
  const { project, selectedImageId, hoveredId, resection, graticule } = args
  const isEmph = (id: string) => id === selectedImageId || id === hoveredId
  ...
  for (const img of project.images) {
    ...
    if (img.subject) {
      subjectPts.push({
        position: [img.subject.lng, img.subject.lat],
        imageId: img.id,
        emphasized: isEmph(img.id),
        unsafe: !img.subject.safeToPublish,
      })
    }
  }
  ...
  // Atlas place (hollow neutral marker).
  if (project.atlas.place) {
    const pl = project.atlas.place
    layers.push(
  ...
  layers.push(
    new ScatterplotLayer({
      ...
      onClick: (info) => {
        ...
        if (obj && args.onPickImage) args.onPickImage(obj.imageId)
      },
      updateTriggers: {
        getRadius: [selectedImageId, hoveredId],
        getLineColor: [selectedImageId, hoveredId],
        getLineWidth: [selectedImageId, hoveredId],
      },
    }),
  )
