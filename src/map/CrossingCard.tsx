import { formatLatLng, type ResectionSet } from '../core'
import { useStore } from '../state/store'

/** Round an adopted crossing to ~0.1 m so it does not carry float noise. */
const round6 = (n: number) => Math.round(n * 1e6) / 1e6

/**
 * The floating readout for the image complex made geometric. When several
 * cameras photograph one object, their bearings cross and resect it. The card
 * reports the strongest crossing, says plainly when the geometry is weak or
 * falls behind a camera, and offers the fix as the atlas place or as the
 * subject of the selected image.
 */
export function CrossingCard({ resection }: { resection: ResectionSet }) {
  const project = useStore((s) => s.project)
  const selectedImageId = useStore((s) => s.selectedImageId)
  const patchAtlas = useStore((s) => s.patchAtlas)
  const updateImage = useStore((s) => s.updateImage)

  const vantageCount = project.images.filter((s) => s.vantage).length
  if (vantageCount < 2) return null

  const best = resection.best
  const point = best?.point

  const adoptAsPlace = () => {
    if (!point) return
    const prev = project.atlas.place
    patchAtlas({
      place: {
        lat: round6(point.lat),
        lng: round6(point.lng),
        safeToPublish: prev?.safeToPublish ?? true,
        name: prev?.name,
      },
    })
  }

  const setAsSubject = () => {
    if (!point || !selectedImageId) return
    const img = project.images.find((s) => s.id === selectedImageId)
    updateImage(selectedImageId, {
      subject: {
        lat: round6(point.lat),
        lng: round6(point.lng),
        safeToPublish: img?.subject?.safeToPublish ?? true,
      },
    })
  }

  return (
    <div className="crossing-card ticks">
      <div className="between">
        <span className="label">Crossing</span>
        <span className="label">{vantageCount} vantages</span>
      </div>

      {best && point ? (
        <>
          <dl className="kv" style={{ marginTop: 8 }}>
            <dt>Fix</dt>
            <dd className="mono">{formatLatLng(point.lat, point.lng)}</dd>
            <dt>Angle</dt>
            <dd className="mono">
              {best.crossingAngleDeg.toFixed(0)}&deg;{' '}
              <span className="faint">
                ({Math.round(best.quality * 100)}% strength)
              </span>
            </dd>
            {resection.points.length > 1 && (
              <>
                <dt>Spread</dt>
                <dd className="mono">{resection.spreadM.toFixed(0)} m</dd>
              </>
            )}
          </dl>

          {best.weak && (
            <div className="warn" style={{ marginTop: 8 }}>
              Rays are close to parallel: this fix is unstable. Treat it as weak
              geometry, not a confident point.
            </div>
          )}

          <div className="btn-row" style={{ marginTop: 10 }}>
            <button className="btn btn-sm btn-primary" onClick={adoptAsPlace}>
              Adopt as atlas place
            </button>
            <button
              className="btn btn-sm"
              onClick={setAsSubject}
              disabled={!selectedImageId}
              title={
                selectedImageId
                  ? 'Set this crossing as the subject of the selected image'
                  : 'Select an image first'
              }
            >
              Set as subject
            </button>
          </div>
        </>
      ) : (
        <div className="note-box" style={{ marginTop: 8 }}>
          The rays do not cross ahead of both cameras. Adjust a bearing or a
          station until they converge on the object.
        </div>
      )}

      <p className="faint" style={{ marginTop: 10, fontSize: 11, lineHeight: 1.4 }}>
        Planar resection at corpus scale. A defensible candidate, not a verified
        geolocation.
      </p>
    </div>
  )
}
