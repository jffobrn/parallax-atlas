...
export async function buildThumbnails(
  ...
  for (const image of project.images) {
    if (image.consent !== 'public') continue
    if (!image.file?.blobKey || !image.file.mime.startsWith('image/')) continue
    const blob = await getMedia(image.file.blobKey)
    if (!blob) continue
    try {
      if (blob.size <= EMBED_AS_IS_LIMIT || image.file.mime === 'image/svg+xml') {
        out[image.id] = await blobToDataUrl(blob)
      } else {
        out[image.id] = await rasterize(blob)
      }
    } catch {
      // If a format cannot be decoded, skip the thumbnail; the entry still lists
