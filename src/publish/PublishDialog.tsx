 ...
 * what the consent boundary removed (including relations and plate tiles that
 * referenced withheld images), previews the resulting self-contained artifact
 * in a sandboxed frame, and offers it for download or print. The preview is the
 * artifact: the same HTML string is shown and saved.
 */
export function PublishDialog({
  ...
  const filename = `${slugify(project.atlas.titles[0]?.text ?? 'atlas')}.html`
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content ticks" style={{ width: 'min(1040px, calc(100vw - 32px))' }}>
          <Dialog.Title className="dialog-title">Publish atlas</Dialog.Title>
          <Dialog.Description className="dialog-desc">
            Everything below passed through the consent boundary. Embargoed and
            restricted images, hidden provenance, unsafe coordinates, and any
            relation or plate tile that referenced them do not cross it.
          </Dialog.Description>
            ...
            <div className="publish-grid">
              <div className="publish-side">
                ...
                <dl className="kv" style={{ marginTop: 8 }}>
                  <dt>Images</dt>
                  <dd className="mono">{pub.images.length} of {project.images.length}</dd>
                  <dt>Relations</dt>
                  <dd className="mono">{pub.relations.length} of {project.relations.length}</dd>
                  <dt>Plates</dt>
                  <dd className="mono">{pub.panels.length} of {project.panels.length}</dd>
                </dl>
                ...
                <p className="faint" style={{ fontSize: 11, marginTop: 10 }}>
                  One self-contained file: plates, image complex, camera
                  positions, chronology, and catalogue. Hosts anywhere or opens
                  offline.
                </p>
              </div>
