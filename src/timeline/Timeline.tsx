...
const M = { top: 14, right: 18, bottom: 24, left: 18 }
...
/**
 * The temporal axis of the atlas: every dated image on one brushable lane,
 * colored by consent, with uncertain times drawn as spans rather than false
 * points. Selecting an image here highlights it in the plate, the graph, and
 * the map; dragging filters the corpus to a window.
 */
export function Timeline() {
  ...
  const laneH = Math.min(20, innerH - 12)
  ...
  if (!extent) {
    return (
      <div className="timeline-wrap" ref={wrapRef}>
        <div className="empty">No dated images yet. Add a datetime to place an image on the chronology.</div>
      </div>
    )
  }

  const win = project.atlas.window
      ...
      <svg width={size.w} height={size.h} style={{ display: 'block' }}>
        <Group left={M.left} top={M.top}>
          ...
          {items.map((it) => {
            ...
            const sel = it.id === selectedImageId
            ...
            return (
              <g
                ...
                onPointerDown={(e) => {
                  e.stopPropagation()
                  useStore.getState().selectImage(it.id)
                }}
